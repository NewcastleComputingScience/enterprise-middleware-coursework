...
* You can use a similar method to add new files or make changes, commit these changes, and push your commits to the remote repository stored on GitHub.

=== Deploying to Azure

Your cloud instance on Azure is set to use Git hooks to automatically deploy your application when you push changes to it. You should have been emailed the DNS name of your instance alongside a password to connect to it.

* Firstly we need to add a new remote repository, directing Git to our VM running in Azure. Make sure you navigate into your cloned Git repository, and run the following:

NOTE: Replace `<dns-name>` name with the one you have been provided with.

    git remote add azure ssh://azure@<dns-name>/var/jboss/jboss-deploy.git

* Whenever you make changes to your code that you would like your cloud deployment to reflect, make sure that all updates are committed to the `master` branch, and simply push to the `azure` remote instead of `origin`:

    git push azure master

* You will need to provide the password that you have been issued to be able to push changes to this repository. You will see the output from Maven running directly on the Azure VM in the same way as when deploying to a local JBoss EAP installation.

Once deployment has completed successfully, you should be able to access your application immediately from `http://<dns-name>:8080`.

TIP: If you are new to Git, you should read link:http://git-scm.com/book[Pro Git].  Chapters 1-3 should cover all the functionality required for this coursework.

TIP: The Azure Git hook detects updates to the master branch, and runs the `mvn wildfly:undeploy` and `mvn clean install wildfly:deploy` actions. If you would like to know more you can read the link:https://git-scm.com/docs/githooks[documentation].
